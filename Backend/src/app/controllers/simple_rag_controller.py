...
class SimpleRAGController(BaseController):
    def __init__(self, db: AsyncSession, request=None):
        super().__init__()
        self.simple_rag_agent_service = SimpleRagAgentService(db, request)

    async def create_agent(self, agent_data, file: Optional[UploadFile] = None):
        try:
            try:
            ...
            creating_process = (
                await self.simple_rag_agent_service.create_simple_rag_agent(
                    agent_data, file
                )
            )
            ...
            return CreateAgentOut(
                id=getattr(creating_process, "id", None),
                name=getattr(creating_process, "name", None),
                avatar=getattr(creating_process, "avatar", None),
                model=getattr(creating_process, "model", None),
                role=getattr(creating_process, "role", None),
                description=getattr(creating_process, "description", None),
                status=getattr(creating_process, "status", None),
                base_prompt=getattr(creating_process, "base_prompt", None),
                short_term_memory=getattr(creating_process, "short_term_memory", None),
                long_term_memory=getattr(creating_process, "long_term_memory", None),
                tone=getattr(creating_process, "tone", None),
                created_at=getattr(creating_process, "created_at", None),
            )
        except Exception as e:
            raise e
