...
class BaseUseCase(ABC, Generic[InputType, OutputType]):
    ...
    def validate_input(self, input_data: InputType) -> UseCaseResult[None]:
        ...
        if input_data is None:
            return UseCaseResult.validation_error("input_data", "Input data cannot be None")
        return UseCaseResult.success_result(None)
    
    async def _execute_with_validation(self, input_data: InputType) -> UseCaseResult[OutputType]:
        """
        ...
        if validation_result.is_error():
            ...
            return UseCaseResult.error_result(error_msg, validation_result.get_exception(), validation_result.get_error_code())
        
        # Execute the use case
        try:
            return await self.execute(input_data)
        except Exception as e:
            return UseCaseResult.error_result(f"Unexpected error in use case: {str(e)}", "INTERNAL_ERROR")