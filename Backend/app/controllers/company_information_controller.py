...
def get_all_company_information(db: Session, current_user: dict) -> List[dict]:
    ...
    try:
        ...
        company_infos = (
            ...
            .options(joinedload(CompanyInformation.agent))
            .all()
        )

        if not company_infos:
            logger.info(
                f"No company information found for user {current_user.get('email')}"
            )
        ...
        raise handle_database_error(
            e, "getting company information", current_user.get("email")
        )


def get_company_information_by_agent_id(
    db: Session, agent_id: int, current_user: dict
) -> dict:
    """
    ...
    try:
        ...
        raise handle_database_error(
            e, "getting company information by agent", current_user.get("email")
        )


def get_company_information_by_id(
    db: Session, company_info_id: int, current_user: dict
) -> dict:
    """
    ...
    try:
        ...
        company_info = (
            db.query(CompanyInformation)
            .join(Agent, CompanyInformation.agent_id == Agent.id)
            .filter(CompanyInformation.id == company_info_id, Agent.user_id == user.id)
            .options(joinedload(CompanyInformation.agent))
            .first()
        )
        ...
        if not company_info:
            logger.info(
                f"Company information {company_info_id} not found or not owned by user {current_user.get('email')}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        raise handle_database_error(
            e, "getting company information by ID", current_user.get("email")
        )
...
def update_company_information(
    ...
    try:
        ...
        try:
            ...
            if agent_id in agents:
                ...
                if hasattr(agents[agent_id], "company_information"):
                    setattr(
                        agents[agent_id], "company_information", updated_company_info
                    )
                    logger.info(
                        f"Updated company_information in agent {agent_id} object"
                    )
                else:
                    ...
                    logger.info(
                        f"Set company_information attribute in agent {agent_id} object"
                    )
            else:
                logger.info(
                    f"Agent {agent_id} not found in memory, skipping agent object update"
                )

        except Exception as agent_update_error:
            # Log error but don't fail the update
            logger.warning(
                f"Failed to update agent object company_information: {str(agent_update_error)}"
            )
        ...
        db.rollback()
        raise handle_database_error(
            e, "updating company information", current_user.get("email")
        )
...
def update_company_information_by_agent_id(
    ...
    try:
        ...
        db.rollback()
        raise handle_database_error(
            e, "updating company information by agent", current_user.get("email")
        )
