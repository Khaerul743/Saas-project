...
def validate_password(password: str) -> str:
    ...
    if password.lower() in ['password', '123456', 'qwerty', 'abc123', 'admin', 'user']:
        raise PasswordTooWeakException("Password is too common, please choose a more secure password")
...
def validate_username(username: str) -> str:
    ...
    if len(username) < 2:
        raise ValidationException("username", "Username must be at least 2 characters long")
    
    if len(username) > 50:
        raise ValidationException("username", "Username must be less than 50 characters")
