...
try:
    ...
    raise ImportError("mem0 library is not installed. Please install it with: pip install mem0ai")
...
class MemoryControl:
    ...
    def get_context(self, query: str, memory_id: str, limit: int = 3) -> str:
        ...
        try:
            ...
            memories = "\n".join(
                [f"- {entry.get('memory', '')}" for entry in get_memory["results"] if entry.get('memory')]
            )
            return memories
    ...
    def delete_memory(self, memory_id: str, memory_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        """
    ...
    def get_all_memories(self, memory_id: str) -> List[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    try:
        ...
        messages = [
            ...
            {"role": "assistant", "content": "Halo John! Senang berkenalan dengan Anda."}
        ]
