...
def create_simple_rag_agent_task(
    ...
    try:
        ...
        publish_agent_event(
            EventType.AGENT_CREATION_PROGRESS, user_id, agent_data["id"], progress
        )
        ...
        new_agent = create_agent_entity(
            agent_data_obj, user_id, agent_role="simple RAG agent"
        )
        ...
        publish_agent_event(
            EventType.AGENT_CREATION_SUCCESS, user_id, agent_data["id"], progress
        )
        ...
        if "Duplicate entry" in str(e) and "PRIMARY" in str(e):
            ...
            logger.warning(
                f"Duplicate agent ID detected: {agent_data.get('id', 'unknown')}"
            )
        ...
        publish_agent_event(
            EventType.AGENT_CREATION_FAILURE,
            user_id,
...
def create_customer_service_agent_task(
    self, files_data: Optional[List[dict]], agent_data: dict, user_id: int
):
    task_id = self.request.id
    db = SessionLocal()
    logger.info(f"Task Execution: user_id {user_id}")
    ...
    try:
        ...
        progress = _update_progress(
            self, 10, status="Initialize Customer Service Agent"
        )
        publish_agent_event(
        ...
        #         # Step 3: Create agent entity
        new_agent = create_agent_entity(
            agent_data_obj, user_id, agent_role="customer service"
        )
        logger.info(f"New agent: {new_agent}")
        db.add(new_agent)
        ...
        #         # Step 9: Success
        progress = _update_progress(
            self, 80, "Customer Service Agent creation completed", "SUCCESS"
        )
        publish_agent_event(
            EventType.AGENT_CREATION_SUCCESS, user_id, agent_data_obj.id, progress
        )
        ...
        logger.error(f"Error creating Customer Service Agent: {str(e)}")
        progress = _update_progress(
            self, 100, "Customer Service Agent creation failed", "FAILURE"
        )
        publish_agent_event(
            EventType.AGENT_CREATION_FAILURE, user_id, agent_data_obj.id, progress
        )
        raise e
    finally:
        db.close()
        ...
        for file_data in files_data:
            try:
                ...
                if content_type in ["pdf", "txt"]:
                    logger.info(
                        f"Processing {content_type} file for RAG: {file_data['filename']}"
                    )
                ...
                _update_progress(
                    task_instance,
                    current_progress,
                    f"Processing file {len(document_ids)}/{len(files_data)}",
                )
                print(
                    "\n============================================================================================================================="
                )
